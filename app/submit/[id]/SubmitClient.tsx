...
export default function SubmitClient({ initialChallenge, initialMySquad }: { initialChallenge: any, initialMySquad: any }) {
                        ...
                        <div className="space-y-4">
                            ...
                            {isSquad && mySquad && (
                                <div className="p-4 rounded-xl bg-indigo-500/[0.05] border border-indigo-500/15 space-y-2">
                                    <p className="text-xs font-medium text-indigo-300">{mySquad.name}</p>
                                    <div className="space-y-1.5">
                                        {mySquad.members.map((m: any) => (
                                            <div key={m.userId} className="flex items-center gap-2 text-xs text-slate-400">
                                                <Check size={10} className="text-emerald-400" /> {m.displayName}
                                            </div>
                                        ))}
                                    </div>
